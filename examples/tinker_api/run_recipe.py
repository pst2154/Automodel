...
RECIPE_DIR = pathlib.Path(__file__).resolve().parent / "recipes"
...
def build_command(recipe: dict[str, Any], overrides: dict[str, Any]) -> list[str]:
    ...
    script = pathlib.Path(__file__).resolve().parent / _script_for_kind(str(recipe["kind"]))
...
def main() -> None:
    ...
    parser.add_argument("recipe", help="Recipe name under examples/tinker_api/recipes or a JSON file path.")
