...
Recipes live in `examples/tinker_api/recipes/` and are dispatched with
`examples/tinker_api/run_recipe.py`.
...
```bash
python examples/tinker_api/run_recipe.py nemotron_sft_large \
